...
def player_search(name)
  players.find do |player| # Select returns and array. Find returns a single entry.
    player.fetch(:player_name)==name
  end
end

def num_points_scored(name)
  player=player_search(name)
  player.fetch(:points)
end
...
def team_colors(team_name)
  specified_team=team_search(team_name)
  specified_team.fetch(:colors)
end
...
def player_numbers(team_name)
  specified_team=team_search(team_name)[:players]
  specified_team.collect do |player|
    player[:number]
  end
end

def player_stats(player_name)
  player_search(player_name).reject do |stat,data|
    stat==:player_name
  end
end
